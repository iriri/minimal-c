...
#define ch_forcesend(c, msg) channel_forcesend(c, &(msg), sizeof(msg))
...
#define ch_set_add(set, c, op, msg) \
    channel_set_add(set, c, op, &(msg), sizeof(msg))
...
#define CHANNEL_EXTERN_DECL \
    ...
    extern inline channel *channel_make(uint32_t, uint32_t); \
    ...
    extern inline channel_rc channel_send(channel *, void *, uint32_t); \
    extern inline channel_rc channel_recv(channel *, void *, uint32_t); \
    extern inline channel_rc channel_trysend(channel *, void *, uint32_t); \
    extern inline channel_rc channel_tryrecv(channel *, void *, uint32_t); \
    extern inline channel_rc channel_timedsend( \
        channel *, void *, uint64_t, uint32_t); \
    extern inline channel_rc channel_timedrecv( \
        channel *, void *, uint64_t, uint32_t); \
    extern inline channel_rc channel_forcesend(channel *, void *, uint32_t); \
    ...
    extern inline uint32_t channel_set_add( \
        channel_set *, channel *, channel_op, void *, uint32_t); \
...
inline channel_rc
channel_send(channel *c, void *msg, uint32_t msgsize) {
...
inline channel_rc
channel_recv(channel *c, void *msg, uint32_t msgsize) {
    ch_assert_(msgsize == c->hdr.msgsize);
    return c->hdr.cap > 0 ?
...
inline channel_rc
channel_trysend(channel *c, void *msg, uint32_t msgsize) {
    ch_assert_(msgsize == c->hdr.msgsize);
    return c->hdr.cap > 0 ?
 ...
 * closed. */
inline channel_rc
channel_tryrecv(channel *c, void *msg, uint32_t msgsize) {
    ch_assert_(msgsize == c->hdr.msgsize);
    return c->hdr.cap > 0 ?
...
inline channel_rc
channel_timedsend(channel *c, void *msg, uint64_t timeout, uint32_t msgsize) {
    ch_assert_(msgsize == c->hdr.msgsize);
...
inline channel_rc
channel_timedrecv(channel *c, void *msg, uint64_t timeout, uint32_t msgsize) {
    ch_assert_(msgsize == c->hdr.msgsize);
    ch_timespec_ ts = channel_add_timeout_(timeout);
    return c->hdr.cap > 0 ?
 ...
 * channel is closed. */
inline channel_rc
channel_forcesend(channel *c, void *msg, uint32_t msgsize) {
...
inline uint32_t
channel_set_add(
    channel_set *s, channel *c, channel_op op, void *msg, uint32_t msgsize
) {
    if (s->len == s->cap) {
        ch_assert_((
